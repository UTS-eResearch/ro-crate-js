...
class ROCrate {
   ...
   toGraph() {
    //turn an entire crate into a linked data structure once this is done you can no longer operate in "old" mode directly with the graph array
        ...
        this.__graphIndex = {};
        this.__graphArray = _.clone(this.getGraph()); // This is still an array
        var g = this.getGraph();
        g  = []; // Save space
        this._item_by_id = {}; // Not to be used any more - we have changed our data structure
   ...
   serializeGraph() {
       const deLink = function (item) {
            ...
            for (let prop of Object.keys(item)) {
                ...
                } else  if (prop === "_reverse") {
                    delete item._reverse;
                    // Do anything?
                } else {
                    const newVals = [];
       ...
       const newGraph = [];
        ...
        for (let item of this.__graphArray){
            idsInGraph.push(item["@id"]);
            newGraph.push(deLink(item));
        }
        // Add in any stray objects 
        for (let item of Object.keys(this.__graphIndex)) {
            if (!idsInGraph.includes(item["@id"])) {
                newGraph.push(deLink(item));
                }
        }
        return newGraph;
   ...
   itemToGraph(item, alreadySeen) {
    const addReverse = function (newItemVal, item, prop) {
        if (!newItemVal._reverse[prop]) {
            newItemVal._reverse[prop] = [];
        }
        newItemVal._reverse[prop].push(item); // Link back to the item we're processing
    ...
    if (!alreadySeen[item["@id"]]) {
        ...
        for (let prop of Object.keys(item)) {
            if (prop === "@type") {
                ...
                delete prop["@rxeverse"]; // Will add our own
            } else if (prop != "@id" && prop != "_reverse") {
                var newVals = [];
